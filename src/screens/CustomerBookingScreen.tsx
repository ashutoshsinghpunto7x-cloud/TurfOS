...
const ALL_MINS = Array.from({ length: 12 }, (_, i) => i * 5);  // 0,5,10,…55
const PERIODS  = ['AM', 'PM'];
const DURATIONS = [
  { label: '30 m',  mins: 30  },
  { label: '1 hr',  mins: 60  },
...
// ─── Drum Picker (White Glassmorphism) ────────────────────────────────────────
const DRUM_H   = 52;
const DRUM_VIS = 3;
const DRUM_PAD = 1;
...
function Drum({ items, selected, onSelect, width = 82 }: DrumProps) {
  ...
  useEffect(() => {
    ...
    if (idx >= 0)
      setTimeout(() => scrollRef.current?.scrollTo({ y: idx * DRUM_H, animated: false }), 80);
  }, []);
  ...
  return (
    <View style={{ width, height: DRUM_H * DRUM_VIS, overflow: 'hidden' }}>
      {/* Selected band — purple gradient pill */}
      <LinearGradient
        colors={GRAD}
        ...
        {items.map((item, i) => {
          ...
          return (
            <View key={String(item) + i} style={{ height: DRUM_H, alignItems: 'center', justifyContent: 'center', zIndex: 1 }}>
              <Text style={{
                fontSize:    isSel ? 26 : 17,
                fontWeight:  isSel ? '800' : '500',
                color:       isSel ? T.white : T.textDis,
...
// ─── Animated Date Card ───────────────────────────────────────────────────────
function DateCard({ date, index, label, isSelected, onPress }: {
  date: Date; index: number; label: string;
    ...
    return (
      <Animated.View style={[dc.wrap, { transform: [{ scale }] }]}>
        <TouchableOpacity onPress={press} activeOpacity={1}>
          <LinearGradient
            colors={GRAD} start={{ x: 0, y: 0 }} end={{ x: 0.5, y: 1 }}
            style={[dc.card, dc.cardSel]}
          >
...
export default function CustomerBookingScreen() {
  ...
  const screen = useBookingScreen(profile?.id ?? null, 'customer', profile?.id ?? null);
  ...
  const handleStartH12 = (v: string | number) => {
    const h = v as number;
    setStartH12(h);
    const h24 = to24h(h, startPer);
    screen.setStartH(h24);
  ...
  useEffect(() => {
    ...
    Animated.parallel([
      ...
      stagger(dateAnim,   100),
      stagger(timeAnim,   200),
      stagger(durAnim,    300),
      stagger(btnAnim,    400),
    ]).start();
  ...
  const selectedDateLabel = screen.selectedDate.toLocaleDateString('en-IN', { weekday: 'long', day: 'numeric', month: 'long' });

  const handleCalSelect = (day: number) => {
    const d = new Date(calYear, calMonth, day);
    if (d < new Date(new Date().setHours(0, 0, 0, 0))) return;
    screen.setSelectedDate(d);
    setCalModal(false);
  };
  ...
  const durLabel = durMins >= 60
    ? `${Math.floor(durMins / 60)}${durMins % 60 ? '.5' : ''} hr`
    : `${durMins} min`;

  // Card date badge (for header)
  ...
  return (
    <View style={s.root}>
        ...
        <KeyboardAvoidingView style={{ flex: 1 }} behavior={Platform.OS === 'ios' ? 'padding' : 'height'}>
          ...
          <Animated.View style={[s.header, {
            ...
            <View style={s.headerMid}>
              ...
              <Text style={s.headerSub}>Choose your perfect slot</Text>
            </View>

            {/* Calendar badge — gradient pill */}
          ...
          >
            ...
            <Animated.View style={{
              opacity: dateAnim,
              transform: [{ translateY: dateAnim.interpolate({ inputRange: [0, 1], outputRange: [16, 0] }) }],
            }}>
              <View style={s.calRow}>
            ...
            <Animated.View style={{
              opacity: durAnim,
              transform: [{ translateY: durAnim.interpolate({ inputRange: [0, 1], outputRange: [16, 0] }) }],
            }}>
              <View style={s.sectionRow}>
                <View style={s.sectionDot} />
              ...
              <View style={s.durRow}>
                {DURATIONS.map(({ label, mins }) => {
                  ...
                  return isSel ? (
                    ...
                    <TouchableOpacity
                      key={mins} style={s.durPill}
                      onPress={() => handleDuration(mins)} activeOpacity={0.75}
                    >
            ...
            {hasError && (
              <View style={s.errBanner}>
                ...
                <Text style={s.errTxt}>
                  {screen.slotConflict?.message ?? screen.timeError}
                </Text>
              </View>
            )}
                ...
                >
                  ...
                  >
                    {screen.acquiringHold
                      ...
                      : (
                        <>
                          <Text style={s.contBtnTxt}>Continue</Text>
                          <Text style={s.contBtnArrow}> →</Text>
                        </>
                      )
                    }
                  </LinearGradient>
      ...
      <View style={[s.navWrap, { paddingBottom: Math.max(insets.bottom, 12) }]}>
        <BlurView intensity={60} tint="light" style={StyleSheet.absoluteFill} />
        <View style={s.navInner}>
          <TouchableOpacity style={s.navItem} onPress={() => navigation.goBack()} activeOpacity={0.7}>
            <Text style={[s.navEmoji, { opacity: 0.55  }]}>🏠</Text>
            <Text style={s.navLbl}>Home</Text>
          </TouchableOpacity>

          <TouchableOpacity style={s.navItem} activeOpacity={0.7}>
            <Text style={[s.navEmoji, { opacity: 0.55 }]}>🔍</Text>
            <Text style={s.navLbl}>Explore</Text>
          </TouchableOpacity>

          {/* Centre FAB */}
          <View style={s.navCenterWrap}>
            <LinearGradient colors={GRAD} start={{ x: 0, y: 0 }} end={{ x: 1, y: 1 }} style={s.navFAB}>
              <Text style={{ fontSize: 24 }}>⚽</Text>
            </LinearGradient>
            <Text style={[s.navLbl, { color: T.navAct, fontWeight: '700' }]}>Book</Text>
          </View>

          <TouchableOpacity style={s.navItem} activeOpacity={0.7}>
            <Text style={[s.navEmoji, { opacity: 0.55 }]}>📅</Text>
            <Text style={s.navLbl}>Bookings</Text>
          </TouchableOpacity>
            ...
            <View style={s.calGrid}>
              ...
              {Array.from({ length: calDaysInMonth }, (_, i) => {
                ...
                return (
                  <TouchableOpacity
                    key={day}
                    style={[s.calCell, isPast && { opacity: 0.28 }]}
                    onPress={() => !isPast && handleCalSelect(day)}
                    disabled={isPast} activeOpacity={0.75}
                  >
                    {isSel && (
                      <LinearGradient colors={GRAD} start={{ x: 0, y: 0 }} end={{ x: 1, y: 1 }}
                        style={s.calSelBg} />
                    )}
                    <Text style={[s.calDayTxt, isSel && { color: T.white, fontWeight: '700' }]}>
                      {day}
                    </Text>
                  </TouchableOpacity>
                );
...
const s = StyleSheet.create({
  ...
  bookedChip: {
    backgroundColor: T.bookedBg, borderRadius: 10,
    paddingHorizontal: 12, paddingVertical: 6,
    borderWidth: 1, borderColor: T.bookedBd,
  },
  bookedChipTxt: { fontSize: 11, fontWeight: '700', color: T.bookedTxt },
  holdChip:      {
    backgroundColor: 'rgba(251,191,36,0.10)', borderRadius: 10,
    paddingHorizontal: 12, paddingVertical: 6,
  ...
  /* ── Date info ── */
  dateInfo:    { flexDirection: 'row', alignItems: 'center', gap: 6, justifyContent: 'center', marginTop: 14 },
  dateInfoIcon:{ fontSize: 13, color: T.text2 },
  dateInfoTxt: { fontSize: 12, color: T.text2, fontWeight: '500' },

  /* ── Error banner ── */
  errBanner: {
    flexDirection: 'row', alignItems: 'center', gap: 8, marginTop: 12,
    backgroundColor: T.errBg, borderRadius: 14, padding: 14,
    borderWidth: 1, borderColor: T.errBd,
  },
  errIcon:{ fontSize: 14, color: T.errTxt },
  errTxt: { flex: 1, fontSize: 12, fontWeight: '600', color: T.errTxt },

  /* ── Slot badge ── */
  slotBadge:      { marginTop: 16, marginBottom: 12, overflow: 'hidden' },
  slotBadgeInner: { flexDirection: 'row', alignItems: 'center', padding: 14, gap: 10, borderRadius: 24 },
  slotIconWrap:   {
  ...
  slotTxt:    { flex: 1, fontSize: 14, fontWeight: '700', color: T.text, letterSpacing: -0.2 },
  slotDurBadge:{ backgroundColor: 'rgba(124,77,255,0.10)', borderRadius: 20, paddingHorizontal: 10, paddingVertical: 4 },
  slotDurTxt:  { fontSize: 11, fontWeight: '700', color: T.grad0 },
  ...
  navWrap: {
    ...
    borderTopWidth: 1, borderTopColor: 'rgba(242,242,246,0.8)',
    overflow: 'hidden',
    backgroundColor: T.navBg,
  },
  navInner: { flexDirection: 'row', alignItems: 'center', paddingTop: 8, paddingHorizontal: 8 },
  navItem:  { flex: 1, alignItems: 'center', gap: 3, paddingVertical: 4 },
  navEmoji: { fontSize: 21 },
  navLbl:   { fontSize: 9, fontWeight: '600', color: T.navInact },
  ...
  calSheet:    {
    ...
    shadowColor: T.grad0, shadowOffset: { width: 0, height: -8 }, shadowOpacity: 0.08, shadowRadius: 32,
  },
  ...
  calCancel:   {
    marginTop: 20, paddingVertical: 15, alignItems: 'center',
